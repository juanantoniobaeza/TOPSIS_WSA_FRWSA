...
% FRWSA_VIKOR(Matrix_C,MM,v,Npoints,Alt_Names,W_Names)
...
function FRWSA_VIKOR(Matrix_C,MM,v,Npoints,Alt_Names,W_Names)
...
for j = 1:length(WMAT(:,1))
    W = WMAT(j,:);
    [ranking,~,~,~] = fun_VIKOR(Matrix_C,W,MM,v); % VIKOR Function Call.
    RankMAT(:,j) = ranking;                        % According to equation (12). Matrix of all rankings calculated with VIKOR function. row(1) = TOP 1; row(n) TOP N. Each column of this matrix is a different ranking performed with a combination of weights corresponding to each row of the WMAT matrix.
end
...
title('VIKOR FRWSA Best Alternative');
legend(Alt_Names);
...
xlabel('Ranking');
ylabel('Alternative');
title('VIKOR FRWSA overall ranking of alternatives')
percentage_format = '%.2f%%';
h.CellLabelFormat = percentage_format;

%% FRWSA PLOT
Rank1row = RankMAT(1, :);             % Top 1 Alternative in each VIKOR simulation. Each column of the Rank1row corresponds to the simulation performed for the combination of weights for each WMatrix row. Rank1row(:,i) -> WMatrix(i,:)
