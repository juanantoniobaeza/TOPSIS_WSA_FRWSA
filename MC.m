...
% MC(Matrix_C, MM,Npoints, Nsim, Alt_Names, Crit_Names)
% % Inputs:
...
function MC(Matrix_C, MM,Npoints, Nsim, Alt_Names, Crit_Names)
%% Number of alternatives and criteria
[N_Alt, N_Crit] = size(Matrix_C);

%% Possible weights
% WMAT calculation using the Monte Carlo (MC) method
G = zeros(Nsim, N_Crit);
for j = 1:Nsim
    G(j, :) = rand(1, N_Crit);
end
...
%% Maximum and minimum values assigned to each weight using the Monte Carlo method
MinW = min(WMAT,[],1);
MaxW = max(WMAT,[],1);
